...
int main(void)
{
    ...
    build_data_limit(&data_ecg, "tests/input_data_ecg_700.txt", "\t", 700, 0, limit_rows);
    build_data_limit(&data_ppg, "tests/input_data_ppg_700.txt", "\t", 700, 0, limit_rows);
    build_data_limit(&data_result, "tests/input_data_result_1.txt", "\t", 0, 1, limit_rows);
        ...
        for (int row = 0; row < data_result.rows; row++)
        {
            for (int i = 0; i < nips; i++)
            {
                if (i < 700)
                    ...
                    net.inputs[i] = data_ppg.in[row][i - 700];
            }
...
//            TRACE_LOG("Outputs: %f %f %f\n", net.outputs[0], net.outputs[1], net.outputs[2]);
        ...
        nn_save(&net, "work_1400-1.net");
    }
    char fname[100];
    snprintf(fname, sizeof(fname), "work_1400-1-%f.net", error / data_result.rows);
    nn_save(&net, fname);
    return 0;
}
